...
import { Alert, AppState,  Dimensions, StyleSheet, Text,  TouchableHighlight, View ,TextInput} from 'react-native';
...
export class SwapPhoneView extends React.Component<IProps, IState> {
  ...
  private getCode() {
    ...
    const params = {
      ...
      type:Body.TypeEnum.NUMBER_4,
    };
    this.props.userStore.requestSwapPhoneVerifyCode(params).then((data: any) => {
      const { verifyCode } = data;
      this.setState({ isEnable:false, isCounting:true });
      showToast(`验证码已发送到您的手机，请注意查收`);
      this.startCountDown(60);
    }).catch((rs) => {
      Alert.alert('温馨提示', '验证码发送失败');
      this.endCountDown();
      this.setState({
        timerCount: 0,
        timerTitle:'获取验证码',
  ...
  public swapAction() {
    ...
    this.props.userStore.requestVerifyOldPhone({ code, mobile }).then(() => {
      clearInterval(this.interval);
      this.props.navigation.navigate('SwapPhoneDetailScreen');
    }).catch(errorHandle);
    ...
    return(
        <View style={{backgroundColor:'#fff',flex:1}}>
            <View style={styles.top} >
              <Text style={styles.leftText}>已绑定手机号</Text>
              <Text style={styles.middleText}>{this.state.mobile}</Text>
              <TouchableHighlight
